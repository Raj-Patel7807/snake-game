    ...
    while (!isGameOver) {
        ...
        for (int y = 1; y < height; y++) {
            for (int x = 0; x <= width + 1; x++) {
                ...
                Point p{x, y};
                if (snake.getHead() == p)
                    cout << Color::brightGreen() << "O" << Color::reset();
                else if (food.getPosition() == p) {
                    if (food.getType() ==
                        FOOD_SPECIAL) { cout << Color::brightYellow() << "$" << Color::reset(); } else if (
                        food.getType() == FOOD_POISON) {
                    ...
                    if (u.type == POWER_SHIELD) { cout << Color::cyan() << "S" << Color::reset(); } else if (
                        u.type == POWER_SLOW) { cout << Color::blue() << "T" << Color::reset(); } else {
        ...
        if (!paused && direction != Direction::STOP) {
            ...
            if (snake.getHead() == food.getPosition()) {
                ...
                if (food.getType() == FOOD_NORMAL || food.getType() == FOOD_SPECIAL || food.getType() ==
                    FOOD_BONUS_TIMED) {
                    snake.grow();
                }
                food.spawn(width, height, buildForbiddenCells(snake, obstacleMgr), elapsedMs);
